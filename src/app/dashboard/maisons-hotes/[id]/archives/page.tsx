...
export default async function GuesthouseArchivesPage({
  params,
  searchParams,
}: {
  ...
  const { search = "", year = "", sort = "checkin_desc" } = await searchParams;
  ...
  const archives = year
    ...
    : allSigned;
  const years = [
    ...new Set(allSigned.map((r) => new Date(r.checkIn).getFullYear())),
  ].sort((a, b) => b - a);
    ...
    <>
      <div className="topbar">
        <div className="topbar-left">
          <div className="topbar-breadcrumb">
            Kordia / {guesthouse.name} / <span>Archives</span>
          </div>
        </div>
      ...
      <div className="content" style={{ maxWidth: "1200px", width: "100%" }}>
        <div className="page-title">
          <h1>
            Contrats <span className="v">signés</span>
          </h1>
          <div className="sub">
            {archives.length} contrat{archives.length > 1 ? "s" : ""} archivé
            {archives.length > 1 ? "s" : ""}
            {year ? ` en ${year}` : ""}
          </div>
        </div>
          ...
          <div className="stat-card green">
            <div className="sc-top">
              ...
              <div className="sc-icon g">
                <Archive size={16} strokeWidth={1.4} color="#4A7353" />
              </div>
            </div>
          ...
          <div className="stat-card violet">
            <div className="sc-top">
              ...
              <div className="sc-icon v">
                <Euro size={16} strokeWidth={1.4} color="#5B52B5" />
              </div>
            </div>
          ...
          <div className="stat-card amber">
            <div className="sc-top">
              ...
              <div className="sc-icon a">
                <TrendingUp size={16} strokeWidth={1.4} color="#8C6A00" />
              </div>
            </div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="card-title">
              <Archive size={16} strokeWidth={1.4} color="#7F77DD" /> Archives
            </div>
          </div>
          ...
          <div className="toolbar">
            <form method="GET" style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              ...
              {search && (
                <Link
                  href={`/dashboard/maisons-hotes/${id}/archives?year=${year}&sort=${sort}`}
                  style={{ fontSize: "12px", color: "var(--ink-lighter)", textDecoration: "none" }}
                >
                  ✕
                </Link>
              )}
            ...
            <div style={{ display: "flex", gap: "4px" }}>
              <Link
                href={`/dashboard/maisons-hotes/${id}/archives?search=${search}&sort=${sort}`}
              ...
              {years.map((y) => (
                <Link
                  key={y}
                  href={`/dashboard/maisons-hotes/${id}/archives?search=${search}&sort=${sort}&year=${y}`}
            ...
            <div style={{ display: "flex", gap: "4px" }}>
              {[
                ...
                <Link
                  key={o.value}
                  href={`/dashboard/maisons-hotes/${id}/archives?search=${search}&year=${year}&sort=${o.value}`}
              ...
              <tbody>
                {archives.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      style={{ textAlign: "center", padding: "56px 20px", color: "var(--ink-lighter)" }}
                    >
                      {search
                        ? `Aucun contrat trouvé pour "${search}"`
                        : "Aucun contrat archivé pour l'instant."}
                    </td>
                  </tr>
                ) : (
                  archives.map((r, i) => {
                    ...
                    const initials =
                      `${r.clientLastName[0] ?? ""}${r.clientFirstName[0] ?? ""}`.toUpperCase();
                    return (
                      <tr key={r.id}>
                        <td>
                          <div className="loc-cell">
                            ...
                            <div>
                              ...
                              <div className="loc-signed">
                                Signé le{" "}
                                {r.contract?.signedAt ? fmtShort(r.contract.signedAt) : "—"}
                                {r.contract?.signedByName
                                  ? ` par ${r.contract.signedByName}`
                                  : ""}
                              </div>
                            </div>
                          </div>
                        ...
                        <td>
                          <span className="loyer">{fmtPrice(r.rent)}</span>
                        </td>
                        <td>
          ...
          <div className="table-footer">
            ...
            {archives.length > 0 && (
              <a
                href={`/api/archives/export-csv?guesthouseId=${id}&year=${year}&search=${search}`}
