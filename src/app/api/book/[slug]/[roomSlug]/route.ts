  ...
  if (notifTo && process.env.RESEND_API_KEY) {
    try {
      ...
      const allergiesHtml = body.allergies
        ? `<p style="font-family:'Plus Jakarta Sans',Helvetica,Arial,sans-serif;font-size:14px;color:#B91C1C;margin:8px 0 0;font-weight:600;">⚠ Allergies : ${escapeHtml(String(body.allergies))}</p>`
        : "";
